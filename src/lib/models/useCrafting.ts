...
export const useCrafting = () => {
  ...
  const searchRecipes = (query: string, recipes: CraftingRecipe[]): CraftingRecipe[] => {
    ...
    return recipes.filter((recipe) => recipe.name.toLowerCase().includes(searchTerm) || recipe.components.some((comp) => comp.name.toLowerCase().includes(searchTerm)));
  };
  ...
  const getRecipesByComponent = (componentName: string): CraftingRecipe[] => {
    return recipes.filter((recipe) => recipe.components.some((comp) => comp.name === componentName));
  };
  ...
  const createRecipe = async (recipeData: any) => {
    ...
    if (
      ...
      !recipeData.category ||
      !recipeData.job ||
      recipeData.experience === null ||
      !recipeData.components ||
      !recipeData.components.length
    ) {
